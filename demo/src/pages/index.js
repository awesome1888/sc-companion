...
`}
    ...
    <h3>media()</h3>
    <p>
      Applies a media query to some CSS. Works in the same manner as{" "}
      <code>grid</code> and <code>cell</code>
    </p>
    <p>Example:</p>
    <SyntaxHL language="javascript" style={docco}>
      {`export const TwoColors = styled.div\`
  \${media({ xs: 'background-color: red', all: 'background-color: blue' }, theme.grid)}
\`;`}
    </SyntaxHL>
...
\`;`}
    ...
    <h3>normalizeCellWidth()</h3>
    <p>
      A fix to{" "}
      <a href="https://stackoverflow.com/questions/50068078/how-to-prevent-flex-items-from-overflowing-flex-parent-with-no-wrap">
        this
      </a>{" "}
      issue.
    </p>
